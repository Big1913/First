...
def pick(paragraphs, select, k):
    ...
    x =[ w for w in paragraphs if select(w)]
    if k <len(x):
        return x[k]
    else:
        return ''
...
def about(subject):
    ...
    def select(paragraph):
        ...
        a = [True for p  in split(lower(remove_punctuation(paragraph))) if p in subject]
        return True in a
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    new_list = [diff_function(typed_word ,w , limit) for w in word_list]
    if min(new_list)> limit:
        return typed_word
    else:
        return word_list[new_list.index(min(new_list))]
...
FINAL_DIFF_LIMIT = 20 # REPLACE THIS WITH YOUR LIMIT
...
def time_per_word(words, timestamps_per_player):
    ...
    for tpp in timestamps_per_player:
        difference.append( [abs(tpp[x] - tpp[x-1]) for x in range(1, len (tpp))])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    new_list = [[] for k in player_indices]
    for i in word_indices:
        ts = [time(match, j, i) for j in player_indices]
        new_list[ts.index(min(ts))].append(get_word(match, i))
    return new_list
    # END PROBLEM 10
