...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    x, y = player_score%10 , opponent_score //10%10
    score  = 3 * abs(y-x) or 1
    return score 
    # END PROBLEM 2
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    if score == sus_points(score):
        return score
    else:
        new_score = sus_points(score)
        return new_score
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_score = 1
    while rolls <=10:
        a = make_averaged(roll_dice , samples_count,)
        b = a(rolls,dice)
        if b == max_score:
            return 1
        if b > max_score:
           max_score = b
           max_score_rolls = rolls 
        rolls = rolls + 1
...
def final_strategy(score, opponent_score):
    ...
    *** The max amount of points form boar brawl I will only roll zero if I get that ***
    """
    # BEGIN PROBLEM 12
    if boar_brawl(score,opponent_score) == 27:
